...
import { buildCurrentUser, CURRENT_USER_KEY } from "./src/lib/userStorage.js";
  ...
  return html`
    <div className="space-y-2">
      <div className="flex items-end gap-3">
        ...
        ${right ? html`<div className="shrink-0">${right}</div>` : null}
      </div>
      <div className="h-px bg-slate-200"></div>
      ${hint ? html`<p className="text-xs text-slate-500">${hint}</p>` : null}
    </div>
  `;
}
...
export default function RegisterPage() {
  ...
  async function handleRegister(e) {
    ...
    try {
      ...
      const user = buildCurrentUser(countryCode, phone);
              ...
              <div className="flex items-end gap-3">
                <div className="shrink-0">
                  ...
                  >
                    ${countryCodes.map(
                      (c) =>
                        html`<option key=${c.code} value=${c.code}>
                          ${c.code} ${c.name}
                        </option>`
                    )}
                  </select>
