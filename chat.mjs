...
export default function ChatPage() {
  ...
  if (!activeId) {
    return html`
      <div className="relative min-h-dvh bg-gray-50 pb-20 text-slate-900">
        <header
          className="fixed inset-x-0 top-0 z-20 border-b border-white/50 bg-white/80 backdrop-blur-md"
        >
              ...
              (t) => html`
                <li key=${t.id}>
                  ...
                  >
                    ...
                    ${t.unread
                      ? html`<span className="mt-2 h-2 w-2 shrink-0 rounded-full bg-orange-500" />`
                      : null}
                  </button>
                </li>
              `
  ...
  return html`
    <div className="relative flex min-h-dvh flex-col bg-gray-50 text-slate-900">
      ...
      <div
        className="mx-auto flex w-full max-w-lg flex-1 flex-col pt-14 pb-[calc(4.25rem+env(safe-area-inset-bottom,0px))]"
      >
        <div
          className="min-h-0 flex-1 space-y-3 overflow-y-auto px-4 py-4 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden"
        >
            ...
            m.from === "me"
              ? html`
                  <div key=${m.id} className="flex justify-end">
                    <div
                      className="max-w-[85%] rounded-2xl rounded-br-md bg-orange-500 px-3.5 py-2 text-[15px] leading-relaxed text-white shadow-sm"
                    >
                      ${m.text}
                    </div>
                  </div>
              ...
              : html`
                  <div key=${m.id} className="flex justify-start">
                    <div
                      className="max-w-[85%] rounded-2xl rounded-bl-md bg-white px-3.5 py-2 text-[15px] leading-relaxed text-slate-800 shadow-sm ring-1 ring-black/[0.04]"
                    >
                      ${m.text}
                    </div>
                  </div>
      ...
      <div
        className="fixed inset-x-0 bottom-0 z-30 border-t border-slate-200/80 bg-white/95 px-3 py-2 pb-[max(0.5rem,env(safe-area-inset-bottom))] backdrop-blur-md"
      >
